# Update a Route Aggregation
Detailed steps for updating an existing route aggregation name, description, or project ID.

## Objective
Update a single route aggregation attribute (`name`, `description`, or `project`) using user-confirmed values.
...
## Definition of Done
- `update_route_aggregation` succeeds with no API errors.
- Route aggregation can be retrieved via `search_route_aggregations` using route aggregation UUID and updated project ID (if project was changed).
- Retrieved configuration reflects the user-confirmed updated field and value.

## Steps

### 1. Resolve Route Aggregation UUID and Project
...
Call `search_route_aggregations` with:
...
If multiple route aggregations match (unexpected), ask the user to select one by UUID, then verify it is the intended target.
...
Require the user to confirm before proceeding to Step 5 (yes/no). The confirmation prompt must explicitly include the selected route aggregation UUID and clearly state the action being performed.
...
### 5. Update the Route Aggregation
Call `update_route_aggregation` once with:
...
### 6. Verify Update
Only proceed if `update_route_aggregation` returned successfully with a route aggregation UUID.

Prepare the verification call:
- Use route aggregation UUID from the update response
- Project ID: use the new project ID if `update_type` was `project`; otherwise use the original project ID
- Pagination: `offset: 0, limit: 1`

Call `search_route_aggregations` with the above parameters.
...
2. On each attempt, check whether the route aggregation is returned and whether requested field changes are reflected.
3. If found and changes are reflected, treat verification as successful and display route aggregation UUID, name, description, project, state, and update timestamp.
...
| Error                                                                     | Action                                                                                                                                                                    |
|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Route aggregation not found (Step 1)                                      | Do not proceed. Ask user to confirm project ID and route aggregation UUID, then re-run `search_route_aggregations`.                                                       |
...
| Route aggregation not found (Step 6 verification)                         | Likely indexing delay. Ask user to retry verification shortly; do not make another write call.                                                                            |
| Route aggregation found but field not yet reflected (Step 6 verification) | Likely indexing delay. Ask user to retry verification shortly; do not make another write call.                                                                            |
